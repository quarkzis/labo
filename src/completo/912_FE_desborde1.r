...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[  , ctrx_quarter_square  := ifelse(ctrx_quarter<0, -(ctrx_quarter^2), ctrx_quarter^2)]
  dataset[  , cpayroll_trx_square  := ifelse(cpayroll_trx<0, -(cpayroll_trx^2), cpayroll_trx^2)]  
  dataset[  , mcaja_ahorro_square  := ifelse(mcaja_ahorro<0, -(mcaja_ahorro^2), mcaja_ahorro^2)]
  dataset[  , mtarjeta_visa_consumo_square  := ifelse(mtarjeta_visa_consumo<0, -(mtarjeta_visa_consumo^2), mtarjeta_visa_consumo^2)]  
  dataset[  , ctarjeta_visa_trx_square  := ifelse(ctarjeta_visa_trx<0, -(ctarjeta_visa_trx^2), ctarjeta_visa_trx^2)]
  dataset[  , mcuentas_saldo_square  := ifelse(mcuentas_saldo<0, -(mcuentas_saldo^2), mcuentas_saldo^2)] 
  dataset[  , mrentabilidad_annual_square  := ifelse(mrentabilidad_annual<0, -(mrentabilidad_annual^2), mrentabilidad_annual^2)]
  dataset[  , mprestamos_personales_square  := ifelse(mprestamos_personales<0, -(mprestamos_personales^2), mprestamos_personales^2)] 
  dataset[  , mactivos_margen_square  := ifelse(mactivos_margen<0, -(mactivos_margen^2), mactivos_margen^2)]
  dataset[  , mpayroll_square  := ifelse(mpayroll<0, -(mpayroll^2), mpayroll^2)] 
  dataset[  , Visa_mpagominimo_square  := ifelse(Visa_mpagominimo<0, -(Visa_mpagominimo^2), Visa_mpagominimo^2)]
  dataset[  , cliente_edad_square  := ifelse(cliente_edad<0, -(cliente_edad^2), cliente_edad^2)] 
  dataset[  , chomebanking_trx_square  := ifelse(chomebanking_trx<0, -(chomebanking_trx^2), chomebanking_trx^2)]
  dataset[  , Visa_msaldopesos_square  := ifelse(Visa_msaldopesos<0, -(Visa_msaldopesos^2), Visa_msaldopesos^2)] 
  dataset[  , Visa_Fvencimiento_square  := ifelse(Visa_Fvencimiento<0, -(Visa_Fvencimiento^2), Visa_Fvencimiento^2)]
  dataset[  , mrentabilidad_square  := ifelse(mrentabilidad<0, -(mrentabilidad^2), mrentabilidad^2)] 
  dataset[  , Visa_msaldototal_square  := ifelse(Visa_msaldototal<0, -(Visa_msaldototal^2), Visa_msaldototal^2)]
  dataset[  , Master_Fvencimiento_square  := ifelse(Master_Fvencimiento<0, -(Master_Fvencimiento^2), Master_Fvencimiento^2)] 
  dataset[  , mcuenta_corriente_square  := ifelse(mcuenta_corriente<0, -(mcuenta_corriente^2), mcuenta_corriente^2)]
  dataset[  , Visa_mpagospesos_square  := ifelse(Visa_mpagospesos<0, -(Visa_mpagospesos^2), Visa_mpagospesos^2)] 
  dataset[  , limite_visa_square  := ifelse(Visa_mlimitecompra<0, -(Visa_mlimitecompra^2), Visa_mlimitecompra^2)]
  dataset[  , limite_master_square  := ifelse(Master_mlimitecompra<0, -(Master_mlimitecompra^2), Master_mlimitecompra^2)]  
  dataset[  , Visa_fechaalta_square  := ifelse(Visa_fechaalta<0, -(Visa_fechaalta^2), Visa_fechaalta^2)]
  dataset[  , mcomisiones_mantenimiento_square  := ifelse(mcomisiones_mantenimiento<0, -(mcomisiones_mantenimiento^2), mcomisiones_mantenimiento^2)]
  dataset[  , Visa_mfinanciacion_limite_square  := ifelse(Visa_mfinanciacion_limite<0, -(Visa_mfinanciacion_limite^2), Visa_mfinanciacion_limite^2)]
  dataset[  , mtransferencias_recibidas_square  := ifelse(mtransferencias_recibidas<0, -(mtransferencias_recibidas^2), mtransferencias_recibidas^2)]
  dataset[  , cliente_antiguedad_square  := ifelse(cliente_antiguedad<0, -(cliente_antiguedad^2), cliente_antiguedad^2)]
  dataset[  , Visa_mconsumospesos_square  := ifelse(Visa_mconsumospesos<0, -(Visa_mconsumospesos^2), Visa_mconsumospesos^2)]
  dataset[  , Master_mfinanciacion_limite_square  := ifelse(Master_mfinanciacion_limite<0, -(Master_mfinanciacion_limite^2), Master_mfinanciacion_limite^2)]
  dataset[  , mcaja_ahorro_dolares_square  := ifelse(mcaja_ahorro_dolares<0, -(mcaja_ahorro_dolares^2), mcaja_ahorro_dolares^2)]
  dataset[  , cproductos_square  := ifelse(cproductos<0, -(cproductos^2), cproductos^2)]
  dataset[  , mcomisiones_otras_square  := ifelse(mcomisiones_otras<0, -(mcomisiones_otras^2), mcomisiones_otras^2)]
  dataset[  , thomebanking_square  := ifelse(thomebanking<0, -(thomebanking^2), thomebanking^2)]
  dataset[  , mcuenta_debitos_automaticos_square  := ifelse(mcuenta_debitos_automaticos<0, -(mcuenta_debitos_automaticos^2), mcuenta_debitos_automaticos^2)]
  dataset[  , mcomisiones_square  := ifelse(mcomisiones<0, -(mcomisiones^2), mcomisiones^2)]
  dataset[  , Visa_cconsumos_square  := ifelse(Visa_cconsumos<0, -(Visa_cconsumos^2), Visa_cconsumos^2)]
  dataset[  , ccomisiones_otras_square  := ifelse(ccomisiones_otras<0, -(ccomisiones_otras^2), ccomisiones_otras^2)]
  dataset[  , Master_status_square  := ifelse(Master_status<0, -(Master_status^2), Master_status^2)]
  dataset[  , mtransferencias_emitidas_square  := ifelse(mtransferencias_emitidas<0, -(mtransferencias_emitidas^2), mtransferencias_emitidas^2)]
  dataset[  , mpagomiscuentas_square  := ifelse(mpagomiscuentas<0, -(mpagomiscuentas^2), mpagomiscuentas^2)]
